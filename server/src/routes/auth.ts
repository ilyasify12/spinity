...
router.post(
  '/register',
  body('username').trim().isLength({ min: 3, max: 24 }).matches(/^[a-zA-Z0-9_.-]+$/).withMessage('Username must be 3-24 chars: letters, numbers, _ . -'),
  body('password').isLength({ min: 6, max: 128 }).withMessage('Password must be 6-128 characters'),
  body('displayName').optional().trim().isLength({ max: 40 }),
