...
router.get('/search', query('q').trim().isLength({ min: 2 }).withMessage('Search needs at least 2 characters'), asyncHandler(async (req, res) => {
...
router.post('/request', body('toUserId').notEmpty(), body('message').optional().isString().isLength({ max: 200 }), asyncHandler(async (req, res) => {
    if (handleValidation(req, res))
        return;
    ...
    if (await Friendship.findOne({ userA: a, userB: b })) {
    ...
    if (reverse) {
        ...
        res.json({ autoAccepted: true, requestId: String(reverse._id) });
        return;
    }
    if (await FriendRequest.findOne({ from: fromId, to: toId, status: 'pending' })) {
...
router.post('/respond', body('requestId').notEmpty(), body('accept').isBoolean(), asyncHandler(async (req, res) => {
    if (handleValidation(req, res))
        return;
    const fr = await FriendRequest.findById(String(req.body.requestId));
    if (!fr || String(fr.to) !== req.userId) {
...
router.post('/cancel', body('requestId').notEmpty(), asyncHandler(async (req, res) => {
    if (handleValidation(req, res))
        return;
    const fr = await FriendRequest.findById(String(req.body.requestId));
    if (!fr || String(fr.from) !== req.userId) {
        res.status(404).json({ error: 'Request not found' });
        return;
    }
    if (fr.status !== 'pending') {
...
router.get('/requests', asyncHandler(async (req, res) => {
    ...
    res.json({
        incoming: incoming.map(r => ({ id: String(r._id), from: r.from, message: r.message, createdAt: r.createdAt })),
        outgoing: outgoing.map(r => ({ id: String(r._id), to: r.to, createdAt: r.createdAt }))
    });
}));
...
router.delete('/:friendId', asyncHandler(async (req, res) => {
    const friendId = String(req.params.friendId);
    const [a, b] = sortedPair(req.userId, friendId);
    await Friendship.deleteOne({ userA: a, userB: b });
    res.json({ ok: true });
}));
router.post('/block', body('userId').notEmpty(), asyncHandler(async (req, res) => {
    if (handleValidation(req, res))
        return;
