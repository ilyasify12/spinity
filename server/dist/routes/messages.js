...
router.get('/:conversationId', query('limit').optional().isInt({ min: 1, max: 100 }), asyncHandler(async (req, res) => {
    const conv = await Conversation.findById(req.params.conversationId);
    if (!conv || !conv.participants.map(String).includes(req.userId)) {
...
router.post('/send', body('kind').isIn(['text', 'playlist', 'track']), body('body').optional().isString().isLength({ max: 4000 }), asyncHandler(async (req, res) => {
    ...
    if ((kind === 'playlist' || kind === 'track') && !payload) {
        res.status(400).json({ error: 'Payload required for share' });
        return;
    }
    if (payload?.tracks && (!Array.isArray(payload.tracks) || payload.tracks.length > 100)) {
        res.status(400).json({ error: 'Too many tracks (max 100)' });
        return;
    }
    ...
    if (!conv) {
        ...
        if (!toUserId) {
            res.status(400).json({ error: 'toUserId or conversationId required' });
            return;
        }
        ...
        if (!conv.participants.map(String).includes(req.userId)) {
